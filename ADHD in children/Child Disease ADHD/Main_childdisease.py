...
main = Tk()
main.title("Children ADHD Disease ")
...
def trainNBC():
    ...
    nbc_cls = GaussianNB()
    nbc_cls.fit(X_train, y_train)
    predict = nbc_cls.predict(X_test)
...
def trainLogisticRegression():
    ...
    text.delete('1.0', END)
    global X_train, X_test, y_train, y_test
    logreg_cls = LogisticRegression(max_iter=1000)
    logreg_cls.fit(X_train, y_train)
    predict = logreg_cls.predict(X_test)
...
font = ('times', 15, 'bold')
title = Label(main, text='Visual and Behavioral Data-Driven ADHD Classification Using Supervised Learning')
...
videoDetectionButton.place(x=20,y=450)
